...
export default function Login() {
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
            msnv: Yup.number().typeError('Vui lòng nhập kí tự số').required('Vui lòng nhập mã số nhân viên').min(7, 'Vui lòng nhập trên 7 chữ số'),
            pass: Yup.string().required('Vui lòng nhập mã số nhân viên').min(8, 'Vui lòng nhập trên 7 chữ số').matches(/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/, 'Mật khẩu chưa đủ mạnh, vui lòng gõ trên kí tự và có kí tự in hoa hoặc đặc biệt'),
        }),
        onSubmit: (values) => {
            console.log(values)
        }
